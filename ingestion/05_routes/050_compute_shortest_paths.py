...
Computes shortest-path routes for all unrouted trips in the database.

For each city:
1. Find trips that have no routes row pointing to a 'shortest' path.
2. Group by unique (origin_node, dest_node) pairs to compute each path once.
3. For each unique O-D pair, compute the shortest path on the OSM graph.
4. Upsert the path into the paths table (deduplicated via partial unique index).
        ...
        if force:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    USING trips t
                    WHERE r.trip_id = t.id AND t.city_id = %s
                    """,
                    (city_id,),
                )
            conn.commit()
...
def main():
    load_dotenv()
    parser = argparse.ArgumentParser(description="Compute shortest paths for unrouted trips")
    ...
    parser.add_argument("--force", action="store_true",
                        help="Delete existing routes rows and recompute from scratch")
    ...
    print("\n🏁 Shortest-path computation finished.")
    conn.commit()
    conn.close()
