...
export default function CompareTrafficChart({ cities, colors }: CompareTrafficChartProps) {
    const [datasets, setDatasets] = useState<(BinData[] | null)[]>([]);
        ...
        cities.forEach((city, i) => {
            ...
            fetchRouteHistogram(city.id)
                .then(series => {
                    ...
                    if (i === 0) {
                        setBinLabels(bins.map(b => b.label));
                    }
                    setDatasets(prev => { const n = [...prev]; n[i] = bins; return n; });
                })
        ...
        >
            ...
            <div className="flex gap-2">
                <div className="flex flex-col-reverse justify-between items-end shrink-0" style={{ height: BAR_HEIGHT }}>
                    {yTicks.map(t => (
                        <span key={t} className="text-[9px] text-gray-400 tabular-nums">{t}</span>
                    ))}
                </div>
                ...
                <div className="flex-1 flex gap-1.5 relative">
                    ...
                    {Array.from({ length: nBins }, (_, binIdx) => (
                        <div key={binIdx} className="flex-1 flex flex-col">
                            ...
                            <span className="text-[9px] text-center text-gray-400 mt-1.5 leading-tight">{labels[binIdx]}</span>
                        </div>
                    ))}
                </div>
            </div>
