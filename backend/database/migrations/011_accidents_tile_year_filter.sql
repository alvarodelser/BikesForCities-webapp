-- Migration 011: add optional year filter to accidents_tile Martin function.
--
...
--   year           — optional; integer year (e.g. 2023) to restrict accidents
...
DECLARE
    ...
    target_year    INTEGER := NULLIF(query_params->>'year', '')::INTEGER;
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE a.city_id = target_city_id
              ...
              AND (target_year IS NULL OR EXTRACT(YEAR FROM a.timestamp)::INTEGER = target_year)
            GROUP BY a.id
        ) a
