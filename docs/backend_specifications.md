...
### 1.1 Core Tables ✅
- [x] **`cities`**: Store city city metadata
...
- [x] **`nodes`**: OSM city nodes with spatial data
  - `id` (BIGINT PRIMARY KEY)
  - `city_id` (INTEGER REFERENCES cities)
  - `geometry` (GEOMETRY(POINT, 4326))
  - `tags` (JSONB)

- [x] **`edges`**: City edges with routing metadata
  - `id` (SERIAL PRIMARY KEY)
  - `city_id` (INTEGER REFERENCES cities)
...
### 2.1 Database Operations (`backend/database/city_io.py`)
...
#### City Management
- [x] `get_or_create_city(name, city, country)` - City CRUD operations
- [x] `get_all_cities()` - List all available cities
- [x] `get_city_by_name(name)` - Retrieve specific city
- [x] `get_city_center(city_id)` - Get city center coordinates

#### Data Insertion
- [x] `put_nodes(city_id, nodes_data)` - Bulk node insertion with conflict handling
- [x] `put_edges(city_id, edges_data)` - Bulk edge insertion with spatial data
- [x] `put_routes(city_id, routes_data)` - Bulk route insertion with deduplication
- [x] `put_features(city_id, features_data)` - Bulk feature insertion

#### Data Retrieval
- [x] `get_nodes(city_id, limit=None, offset=None)` - Paginated node retrieval
- [x] `get_edges(city_id, limit=None, offset=None)` - Paginated edge retrieval
- [x] `get_routes(city_id, limit=None, offset=None)` - Paginated route retrieval
- [x] `get_features(city_id, feature_type=None, limit=None, offset=None)` - Retrieve features

#### Statistics & Analytics
- [x] `count_nodes(city_id)` - Node count for city
- [x] `count_edges(city_id)` - Edge count for city
- [x] `count_routes(city_id)` - Route count for city
- [x] `count_features(city_id, feature_type=None)` - Feature count by type
- [x] `get_city_bounds(city_id)` - Geographic bounding box

### 2.2 City Processing (`backend/processing/city_ops.py`)

#### Graph Operations
- [x] `load_graph(place_name, network_type='bike')` - OSMnx graph download with caching
- [x] `build_graph(city_id)` - Reconstruct NetworkX graph from database
...
#### Feature Extraction
- [x] `extract_features_for_network(city_id, distance=10000)` - Extract all features from OSM using `ox.features_from_point()`
...
#### Batch Processing
- [x] `process_all_csvs(data_dir, city_id)` - Process multiple CSV files
- [x] `process_single_csv(csv_path, city_id)` - Process individual CSV file
- [x] `load_next_csv(data_dir, city_id)` - Resume processing from checkpoints
...
#### City Visualization
- [x] `plot_network_overview(cities)` - Multi-city comparison charts
- [x] `plot_network_graph(city_id, sample_size=None)` - Geographic city plots
- [x] `plot_cycleway_network(city_id)` - Dedicated bike infrastructure
- [x] `plot_highway_distribution(city_id)` - Highway type analysis

#### OSM Feature Visualization ✅
- [x] `plot_features_map(city_id, boundary_settings, features_data, save_path=None)` - Multi-layer feature maps
- [x] `generate_features_map(city_id, boundary_settings, save_path=None)` - Generate complete feature maps
- [x] `plot_features_overview(city_id)` - Feature statistics overview
- [x] `load_features_from_db(city_id, boundary_polygon=None)` - Load and filter features
...
- [x] `print_network_stats(city_id)` - Comprehensive city statistics
...
### 3.1 City Setup (`ingestion/01_load_cities.py and ingestion/02_load_osm.py`) ✅
- [x] **Interactive City Selection**: Choose from spain_data.json configuration
- [x] **OSM Data Download**: Automated city download with progress tracking
- [x] **Database Population**: Bulk insertion with performance timing
- [x] **Feature Extraction**: Extract and store OSM features during city setup
- [x] **Error Handling**: Graceful handling of download and insertion failures

### 3.2 Trip Processing (`ingestion/03_load_trips.py`) ✅
...
- [x] **City Overview**: List all cities with statistics
...
- [x] **Multi-city Support**: Handle multiple cities in single database
...
- [x] **Configuration Options**:
  ...
  - `--city`: Specify city for visualization
...
GET    /api/cities/{id}/nodes         # Get city nodes (paginated, bbox)
GET    /api/cities/{id}/edges         # Get city edges (paginated, filters)
GET    /api/cities/{id}/routes        # Get routes with filtering (paginated)

# OSM Features
GET    /api/cities/{id}/features      # Get features by city (paginated, filters)
GET    /api/cities/{id}/features/geojson  # Export as GeoJSON
